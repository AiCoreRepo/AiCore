...
export class ReturnService {
  ...
  async markPickedUp(returnId: string) {
    ...
    if (returnRequest.return_status !== ReturnStatus.PICKUP_SCHEDULED) {
      throw new BadRequestException('Return must be in PICKUP_SCHEDULED state');
    }
  ...
  async completeQC(returnId: string, dto: CompleteQCDto) {
    ...
    if (returnRequest.return_status !== ReturnStatus.PICKED_UP) {
      throw new BadRequestException('Return must be picked up before QC');
    }
    ...
    this.logger.log(
      `QC completed for return ${returnId}: ${dto.qc_passed ? 'PASSED' : 'FAILED'}`,
    );
    ...
    if (dto.qc_passed) {
      try {
        ...
        this.logger.error(
          `Failed to initiate refund for return ${returnId}`,
          error,
        );
      }
    }
  ...
  async completeReturn(returnId: string) {
    ...
    if (returnRequest.return_status !== ReturnStatus.QC_PASSED) {
      throw new BadRequestException('Return must pass QC before completion');
    }

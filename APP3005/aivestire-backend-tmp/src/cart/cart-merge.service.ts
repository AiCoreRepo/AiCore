...
export class CartMergeService {
    ...
    async mergeGuestCartToUser(
        ...
        this.logger.log(`Starting merge: user=${userId}, session=${guestSessionId}`);
        ...
        const guestItems = await this.guestCartService.getGuestCartItemsForMerge(guestSessionId);
        ...
        await this.prisma.$transaction(async (tx) => {
            for (const guestItem of guestItems) {
                ...
                if (product.is_deleted || product.status !== CART_CONSTANTS.APPROVED_STATUS) {

...
export class CartController {
  ...
  async updateGuestCartItem(
    ...
    return this.guestCartService.updateGuestCartItem(
      sessionId,
      cartItemId,
      dto,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Called after login to merge any guest cart items into the authenticated user cart. This is idempotent - calling multiple times produces the same result.',
  })
  @ApiResponse({
    status: 200,
  ...
  async mergeCart(@Request() req, @Req() rawReq) {
    ...
    return this.cartMergeService.mergeGuestCartToUser(
      req.user.user_id,
      sessionId,
    );
  }

...
export class GuestCartService {
     ...
     */
    async addToGuestCart(sessionId: string, dto: AddToCartDto): Promise<GuestCartResponse> {
        ...
        if (product.inventory_count < quantity) {
            throw new BadRequestException(
                CART_MESSAGES.INSUFFICIENT_INVENTORY.replace('{count}', product.inventory_count.toString()),
            );
        }
    ...
    async updateGuestCartItem(
        ...
        if (cartItem.product.inventory_count < dto.quantity) {
            throw new BadRequestException(
                CART_MESSAGES.INSUFFICIENT_INVENTORY.replace('{count}', cartItem.product.inventory_count.toString()),
            );
        }
     ...
     */
    async removeFromGuestCart(sessionId: string, cartItemId: string): Promise<GuestCartResponse> {
        const cartItem = await this.prisma.guestCartItem.findUnique({
            where: { guest_cart_item_id: cartItemId },
            select: {
                guest_cart_item_id: true,

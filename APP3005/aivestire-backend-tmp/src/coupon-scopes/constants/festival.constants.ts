...
export const FESTIVAL_KEYS: string[] = FESTIVALS.map(f => f.key);

export const isValidFestivalKey = (key: string): boolean => {
    return FESTIVAL_KEYS.includes(key);
};

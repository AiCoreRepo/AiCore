...
export class AdminSeederService implements OnModuleInit {
  ...
  private async seedAdminIfConfigured(): Promise<void> {
    ...
    if (!adminEmail || !adminPassword) {
      this.logger.debug(
        'Admin seeding skipped: SEED_ADMIN_EMAIL or SEED_ADMIN_PASSWORD not set',
      );
      return;
    }
    ...
    try {
      ...
      if (existingUser) {
        ...
        if (existingUser.role !== 'ADMIN') {
          ...
          this.logger.log(
            `✅ Existing user ${adminEmail} upgraded to ADMIN role`,
          );
        } else {
      ...
      this.logger.warn(
        `   ⚠️ IMPORTANT: Remove SEED_ADMIN_EMAIL and SEED_ADMIN_PASSWORD env vars after confirming login works!`,
      );
    } catch (error) {

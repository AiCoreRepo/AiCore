...
export abstract class BaseTryOnService {
    ...
    async processTryOn(
        ...
        try {
            ...
            this.logger.log(
                `Try-on completed successfully in ${processingTimeMs}ms`,
            );
    ...
    protected async validateImages(
        ...
        if (!avatarValidation.valid) {
            throw new Error(`Avatar image validation failed: ${avatarValidation.error}`);
        }
    ...
    protected async performTryOnWithRetry(
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                lastError = error;
                this.logger.warn(
                    `Try-on attempt ${attempt} failed: ${error.message}`,
                );

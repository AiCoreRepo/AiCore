...
export class DirectVertexTryOnService extends BaseTryOnService {
    ...
    constructor(
        ...
        this.location = this.configService.get<string>('VERTEX_LOCATION') || 'us-central1';
        this.modelId = this.configService.get<string>('VERTEX_MODEL_ID') || 'virtual-try-on-preview-08-04';
    ...
    private async convertToJpeg(base64Data: string): Promise<string> {
        ...
        try {
            ...
            this.logger.debug(`✅ Converted to JPEG: ${buffer.length} -> ${jpegBuffer.length} bytes`);
    ...
    protected async performTryOn(
        ...
        if (!this.isConfigured()) {
            throw new ConfigurationException(
                ...
                { projectId: this.projectId, hasCredentials: !!this.serviceAccountCredentials },
            );
        }
        ...
        const avatarClean = await this.convertToJpeg(this.cleanBase64Data(avatarBase64));
        const clothingClean = await this.convertToJpeg(this.cleanBase64Data(clothingBase64));
        this.logger.log(`✅ Images processed (avatar: ${avatarClean.length} chars, clothing: ${clothingClean.length} chars)`);
        ...
        const payload = {
            ...
            parameters: {
                ...
                baseSteps: additionalParams?.base_steps ?? 30,
        ...
        try {
            ...
            this.logger.log('✅ DIRECT VERTEX AI - Virtual try-on completed successfully!');
    ...
    async isAvailable(): Promise<boolean> {
        ...
        try {
            ...
            this.logger.warn(`Direct Vertex AI service health check failed: ${error.message}`);
            return false;
        }
    }
    ...
    getConfigurationStatus(): { configured: boolean; message?: string } {
        ...
        if (!this.serviceAccountCredentials) {
            return {
                configured: false,
                message: 'Service account credentials not loaded. Set GOOGLE_APPLICATION_CREDENTIALS or place service_account.json in project root',
            };
        }

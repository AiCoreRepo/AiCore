...
export class BodyAnalyzerService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.fastApiEnabled = !!this.configService.get<string>(
      'FASTAPI_BODY_ANALYZE_URL',
    );

    if (this.fastApiEnabled) {
      this.logger.log(
        `✅ Body Analyzer FastAPI service configured at: ${this.fastApiUrl}`,
      );
    } else {
      this.logger.warn(
        '⚠️  FASTAPI_BODY_ANALYZE_URL not configured, using default localhost:8000',
      );
    }
  }
  ...
  private async handleFastApiError(response: Response): Promise<never> {
    ...
    this.logger.error(
      `FastAPI Body Analyzer error (${statusCode}): ${JSON.stringify(errorData)}`,
    );

...
import { GEMINI_MODEL_ID, GENERATION_TIMEOUT } from '../constants/angle.constants';
...
export class AngleGenerationService {
    ...
    async generateAngle(
        ...
        try {
            ...
            if (!cachedMetadata && request.previousImageUrl.includes('cloudinary.com')) {
                cachedMetadata = await this.extractCachedMetadata(
            ...
            this.logger.log(
                `✅ Angle generation completed in ${processingTime}ms`,
            );
    ...
    private async uploadAndSave(
        ...
        try {
            ...
            // Extract metadata
            const imageMetadata = await this.imageOptimizer.extractImageMetadata(imageData);

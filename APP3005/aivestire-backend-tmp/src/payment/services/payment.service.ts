...
export class PaymentService {
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.logger.error(
        `Webhook processing error for ${event}: ${error.message}`,
      );
   ...
   */
  async initiateRefund(userId: string, dto: InitiateRefundDto): Promise<any> {
  ...
  private async handlePaymentCaptured(paymentEntity: any): Promise<void> {
    ...
    this.logger.log(
      `Webhook: Payment captured for order ${transaction.order_id}`,
    );
  }
  ...
  private async handlePaymentFailed(paymentEntity: any): Promise<void> {
    ...
    this.logger.warn(
      `Webhook: Payment failed for order ${transaction.order_id}`,
    );
  }
  ...
  private async handleOrderPaid(orderEntity: any): Promise<void> {
    ...
    this.logger.log(
      `Webhook: Order paid event for Razorpay order ${orderEntity.id}`,
    );

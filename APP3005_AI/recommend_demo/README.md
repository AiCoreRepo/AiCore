...
## Use main_train_data.csv with a held-out test set

Split the main training CSV into train/test and convert it to the fusion format:

```bash
uv run python scripts/prepare_fusion_training_from_main.py --input main_train_data.csv --split 0.8
```

Train on the train split:

```bash
uv run python scripts/train_fusion_mlp.py --data training_main_train.csv --artifacts artifacts
```

Evaluate on the test split:

```bash
uv run python scripts/evaluate_fusion_mlp.py \
  --data training_main_test.csv \
  --model artifacts/fusion_mlp.pt \
  --preprocess artifacts/fusion_preprocess.json
```

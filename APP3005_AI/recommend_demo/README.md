...
This script uses Sentence-BERT for text embeddings, OpenCLIP for image embeddings, and an MLP
fusion head on the concatenated vector (image + text + attributes).
It requires heavier deps (torch, sentence-transformers, open_clip_torch).

## To enable training later
Add heavy deps (torch/torchvision/faiss/open_clip_torch, etc.) back to `pyproject.toml` and rebuild.
The existing training/recommender scripts will then work.

Note: CPU-only torch wheels are used in this repo (`torch==2.2.0+cpu` / `torchvision==0.17.0+cpu`).
If you install locally, set `UV_EXTRA_INDEX_URL=https://download.pytorch.org/whl/cpu` (or `PIP_EXTRA_INDEX_URL`)
so the CPU wheels can be resolved.
